    ...
    <aside className="border-b border-slate-800/80 bg-slate-950/95 px-4 py-4 backdrop-blur md:sticky md:top-0 md:h-screen md:w-72 md:border-b-0 md:border-r md:px-5 md:py-6">
      <div className="mb-4 flex items-center justify-between md:mb-10 md:flex-col md:items-start md:gap-4">
        <Link href={buildRolePath(role, "/dashboard")} className="flex items-center gap-3">
          <span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-cyan-500/20 text-lg font-semibold text-cyan-300 ring-1 ring-inset ring-cyan-400/30">
            E
          </span>
          <span>
        ...
        <span className="rounded-full border border-cyan-400/20 bg-cyan-500/10 px-3 py-1 text-xs font-medium capitalize text-cyan-200">
          {role} view
        </span>
      </div>
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`min-w-fit rounded-2xl px-4 py-3 text-sm font-medium transition ${
                active
      ...
      <div className="mt-6 hidden rounded-3xl border border-white/10 bg-white/5 p-4 md:block">
        ...
        <p className="mt-2 text-sm text-slate-400">
          {role === "admin"
            ...
            : role === "teacher"
              ? "Review courses, publish assignments, and grade submissions in one place."
              : "Track enrolled courses, submit work, and follow grades as they are posted."}
        </p>
      </div>
    </aside>
  );
